...
# 서기 1년1월1일부터 입력한 날짜까지의 누적일수 합산
total_days = 0
'''
월별 날짜수를 리스트로 정의한다. 1월을 인덱스 1로 사용하기 위해 첫번째요소는 0으로 설정한다.
'''
...
'''
입력 년도의 각 월의 날짜수를 누적해서 합산. 만약 1월을 입력했다면 해당 for문은
실행되지 않는다.'''
...
윤년은 2월29일까지 있기 때문이다. 만약 1,2월을 입력했다면 이 부분은 고려할 필요가 없다.'''
if month >=3:
  if year % 400 == 0:
    total_days += 1
  elif year % 100 == 0:
...
'''
서기 1년1월1일은 월요일이므로 7로 나눈 나머지를 통해 요일을 판단할 수 있다.
나누어 떨어지면 일요일, 1이 남으면 월요일로 판단한다.'''
