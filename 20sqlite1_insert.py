...
예외처리 하지 않으면 실행이 중지된다. 혹은 if not exist를 쿼리문에
추가해도 된다.'''
try:
...
# 레코드 2개 이상 삽입. 튜플로 구성된 리스트 사용.
curs.executemany('insert into people values (?,?,?)', [(f'강감찬{rnd}','장군',700),(f'홍길동{rnd}','의적', 800)])

# for문을 이용한 삽입.
rows = [[f'곽재우{rnd}','의병',1100], [f'유성룡{rnd}','재상',1200], [f'임진석{rnd}','의적',1500]]
# 리스트의 크기만큼 반복해서 입력
for row in rows:
