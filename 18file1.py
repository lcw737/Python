'''
open()
: 파일을 다룰때 사용하는 내장함수로 첫번째 인자인 file경로만 필수사항이고
  나머지는 옵션이다.
  형식] open(파일경로, mode, encoding)
  mode : 파일 열기 모드로 w(쓰기), r(읽기), a(추가)가 있고 b(이진모드), t(텍스트모드)로
         파일의 형식을 지정할 수 있다.
'''
print("="*30)
print("내파일01.txt")
print("="*30)
'''
새로운 파일을 생성하여 반복문으로 내용을 입력. wt이므로 쓰기/텍스트
모드로 파일오픈.
'''
...
# 파일의 길이는 알 수 없으므로 무한루프 구성
while True:
  # 파일의 내용을 한줄씩 읽음
  line = f_read.readline()
  # 더이상 읽을 내용이 없다면 반복문 탈출
  if not line: break
  print(line)
# 작업을 마쳤다면 자원해제
f_read.close()
        
# 기존파일에 내용을 추가하기 위해 추가/텍스트 모드로 오픈
